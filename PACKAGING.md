...
python tools/export_onnx.py --model google/siglip2-base-patch16-224 \
                            --out models/siglip2-base --quantize
...
The standard release contains only the INT8 graphs; set
`PHOTOLIB_MODEL_VARIANT=fp32` for an explicit maximum-quality comparison
build. Windows uses a Tauri NSIS setup executable as its single supported
installer. It installs for the current user by default and supports a quiet
...
| | |
|---|---|
| Frozen Python/ONNX/face runtime | ~732 MiB in an isolated Windows build |
| SigLIP 2 base, ONNX fp32 | ~1.54 GB uncompressed |
| SigLIP 2 base, hybrid INT8 (`--quantize`) | ~827 MiB including tokenizer and golden vectors |
| Tracked web UI | ~25 KB |
| Face weights (downloaded on first use) | ~290 MB |
| Windows NSIS setup executable | ~838 MiB in an isolated release build |

The hybrid profile quantizes the large token table and final vision blocks,
while keeping the quality-sensitive vision path in FP32. The exporter enforces
at least 0.98 cosine parity against golden vectors. Releases should still be
checked against a labelled retrieval set: vector parity cannot prove that
ranking is good enough for every photo library.
